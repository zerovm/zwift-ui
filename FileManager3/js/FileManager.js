...
document.addEventListener('keydown', function (e) {
	...
	if (e.target.id == 'input_CreateContainerDialog') {

		if (e.which == 13) {
			FileManager.CreateContainerDialog.ok();
			return;
		}

		FileManager.CreateContainerDialog.clearErrors(e.target);
	}
	...
	if (e.target.classList.contains('create-directory-input')) {
		...
		if (e.which == 13) {
			FileManager.CreateDirectoryDialog.click();
			return;
		}

		FileManager.CreateDirectoryDialog.clearErrors(e.target);
	}
	...
	if (e.target.classList.contains('create-file-input-name')) {
		...
		FileManager.CreateFileDialog.clearErrors(e.target);
	}

	if (e.target.classList.contains('create-file-input-type')) {

		if (e.which == 13) {
			FileManager.CreateFileDialog.click();
			return;
		}
